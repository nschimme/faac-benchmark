...
class TestE2EMock(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for corpus in CORPORA.values():
            os.makedirs(os.path.join(self.data_dir, corpus["dir"]), exist_ok=True)
    ...
    def _create_corpus(self):
        # One fixture set per corpus, in that corpus's declared format, so a
        # newly added corpus is covered without touching this test.
        for corpus in CORPORA.values():
            for i in range(SAMPLE_COUNT):
                write_wav(os.path.join(self.data_dir, corpus["dir"], f"sample_{i}.wav"),
                          sr=corpus["rate"], ch=corpus["channels"])
    ...
    def _assert_base_results(self, data: dict):
        ...
        self.assertTrue(any(k.startswith("16k_mono_20k_") for k in data["matrix"]),
                        "expected 16k_mono_20k keys in base results")
        self.assertFalse(any(k.startswith("24k_mono_32k_") for k in data["matrix"]),
                         "24k_mono_32k should not appear in base results")
    ...
    def _assert_cand_results(self, data: dict):
        ...
        self.assertTrue(any("16k_mono_20k_" in k for k in keys), "16k_mono_20k scenario missing")
        self.assertTrue(any("24k_mono_32k_" in k for k in keys), "24k_mono_32k scenario missing")
        self.assertTrue(any("44k1_stereo_128k_" in k for k in keys), "44k1_stereo_128k scenario missing")
        for mk in (k for k in keys if "44k1_stereo_128k_" in k):
